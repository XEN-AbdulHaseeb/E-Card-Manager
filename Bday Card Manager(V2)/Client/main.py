...
shake = Animation(x=198,duration = .05) + Animation(x=202,duration = .05) + Animation(x=198,duration = .05) + Animation(x=202,duration = .05) + Animation(x=200,duration = .05)
emailRegEx = r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b'

class PreviewScreen(BoxLayout,Screen):
    img_src = StringProperty('/home/xen/Desktop/BirthDayBOT-main/ClientBDBot/CustomImg/Template_1.jpg')
    ...
    def scanimg(self,root_manager):
        if self.CustomImgLoaded == 0: #Dont want to keep on adding same widgets everytime user clicks "Advanced"
            ...
            filelist = glob.glob('/home/xen/Desktop/BirthDayBOT-main/ClientBDBot/CustomImg/*.jpg')
            tmpfilelist = glob.glob('/home/xen/Desktop/BirthDayBOT-main/ClientBDBot/CustomImg/*.jpeg')

            #for i in tmpfilelist:
             #   filelist.append(i)
            for i in range(len(filelist)):
                tmp = ToggleButton(background_color = [1,1,1,.5],background_normal=filelist[i],background_down = filelist[i], group='img')
            ...
            '''Scroll range of Scrollview depends on the height property of child widget, needed to scale GridLayout's height property with the no. of                         rows present'''
    ...
    def enterData(self,Year,Month,Day,ToEmail):
        ...
        try:
            ...
            try:
                ...
                s = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
                s.connect((IP,1338)) #Cant reopen socket on port 1337 as it was closed earlier
    ...
    def defaults(self,root_manager):
        global customText
        customText = ''
        self.img_src = '/home/xen/Desktop/BirthDayBOT-main/ClientBDBot/CustomImg/Template_1.jpg'
...
class DetailInputScreen(BoxLayout,Screen):
    ...
    TextFieldAccessCount = NumericProperty(0) #Dont want to prompt user to enter a valid entry as soon as they try to enter data
    ...
    def load_font(self,root_manager):
        customBirthdayCard(root_manager)
        fontlist = glob.glob('/home/xen/Desktop/BirthDayBOT-main/ClientBDBot/CustomFonts/*.ttf')
        for i in fontlist:
            #root_manager.get_screen('previewscreen').ids.FontList.values.append(i)
            i = re.sub('/home/xen/Desktop/BirthDayBOT-main/ClientBDBot/CustomFonts/','',i)
            i = re.sub('.ttf','',i)
            root_manager.get_screen('previewscreen').ids.FontList.values.append(i)
            print(i)
...
def customBirthdayCard(root_manager):
    ...
    title_font = ImageFont.truetype(f'/home/xen/Desktop/BirthDayBOT-main/ClientBDBot/CustomFonts/{Font}.ttf', int(FontSize))
    ...
    """ For some reason kivy was loading the previous state of the image with the same name after being overwritten with a different
        template, prolly cuz it was using some cache to load the image, so I changed the name each time an image was overwritten with a template
        by appending it with an unrelated item like an integer"""
